...
# source("/home/bstock/Documents/ms/2D-AR1-survival/code/bias_correct_oepe/1i_fit_models_M_CPI.R")
...
df.mods <- data.frame(M_re = c('iid','2dar1'),
                      NAA_cor = c('iid','iid'), stringsAsFactors=FALSE)
n.mods <- dim(df.mods)[1]
df.mods$Model <- paste0("M-CPI-",1:n.mods)
...
for(m in 1:n.mods){
  ...
  input <- prepare_wham_input(asap3, recruit_model = 3, # Bev Holt recruitment
                              ...
                              NAA_re = list(cor=df.mods$NAA_cor[m], sigma="rec"),
  ...
  saveRDS(mod, file=here("results","dat_2019","bias_correct_oepe_rev","M_CPI",paste0(df.mods$Model[m],".rds")))
...
mod.list <- here("results","dat_2019","bias_correct_oepe_rev","M_CPI",paste0(df.mods$Model,".rds"))

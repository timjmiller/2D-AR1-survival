...
# source("/home/bstock/Documents/ms/2D-AR1-survival/code/1f_fit_models_NAA_M_GSI.R")
...
for(m in 1:n.mods){
  ...
  ecov <- list(
    ...
    logsigma = "est_1",
    # logsigma = matrix(log(0.1), ncol=1, nrow=dim(gsi)[1]),
    year = gsi$Year,
...
# input$map = input$map[!(names(input$map) %in% c("index_paa_pars", "catch_paa_pars"))]  
  ...
  saveRDS(mod, file=here("results","dat_2019","NAA_M_GSI_ar1",paste0(df.mods$Model[m],".rds")))
}

...
# source("/home/bstock/Documents/ms/2D-AR1-survival/code/bias_correct_oe/2h_results_NAA_CPI.R")
...
write.csv(df.mods, file=here("tables","dat_2019","bias_correct_oe","NAA_CPI.csv"))

# dAIC boxplot, group by GSI-Rec model
# point: 2D AR1 does best no matter how you model Recruitment/GSI
df <- read.csv(here("tables","dat_2019","NAA_CPI.csv"), stringsAsFactors=F)[,-1]
...
df <- df %>% group_by(CPI_mod, CPI_how) %>% mutate(minAIC_group=min(AIC, na.rm=T)) %>% mutate(dAIC_group=AIC-minAIC_group) %>% as.data.frame

png(here("plots","NAA_CPI_dAIC_boxplot.png"), units='in', height=5, width=7, res=300)

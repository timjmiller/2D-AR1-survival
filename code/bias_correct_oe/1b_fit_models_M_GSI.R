...
# source("/home/bstock/Documents/ms/2D-AR1-survival/code/1b_fit_models_M_GSI.R")
...
# input$map = input$map[!(names(input$map) %in% c("index_paa_pars", "catch_paa_pars"))]
  ...
  if(opt_conv){
    n.try <- 1
    while(!ok_sdrep & n.try < 6){
      n.try = n.try + 1
      # jitter pars and try again
      new_input <- input
      new_input$par <- mod$opt$par + rnorm(length(mod$opt$par), 0, 0.2)  
    ...
    if(ok_sdrep){ # do retro + proj if pdHess = TRUE
      new_input <- input
      new_input$par <- mod$opt$par
      mod <- fit_wham(new_input, do.retro=T, do.osa=F, do.proj=T)
    }
  }
  saveRDS(mod, file=here("results","dat_2019","NAA_M_GSI",paste0(df.mods$Model[m],".rds"))) 
}

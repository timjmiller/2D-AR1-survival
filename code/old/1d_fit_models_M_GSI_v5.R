...
# source("/home/bstock/Documents/ms/2D-AR1-survival/code/1d_fit_models_M_GSI_v5.R")
...
for(m in 1:n.mods){
  ...
  if(df.mods[m,"est_M"]){
    M_list <- list(model="constant", est_ages=1, re=df.mods$M_re[m], initial_means=0.28)
  } else {
...
# input$map = input$map[!(names(input$map) %in% c("index_paa_pars", "catch_paa_pars"))]
  ...
  while(!ok_sdrep & n.try < 11){
    ...
    if(n.try %% 2 == 0) new_input$par$logit_selpars[1,7:8] = c(-1.5,-5) + rnorm(2,0,0.2)
  }
  saveRDS(mod, file=here("results","dat_2019","NAA_M_GSI_v5",paste0(df.mods$Model[m],".rds"))) 

  if(ok_sdrep){ # do retro + proj if pdHess = TRUE
    new_input <- input 
    new_input$par <- mod$env$parList(mod$opt$par) # start at optimized values
    # new_input$par <- mod$env$parList(mod$input$par) # start at same initial values as before (refit model)
    mod2 <- fit_wham(new_input, do.retro=T, do.osa=F, do.proj=T)
    if(exists("err")) rm("err") # need to clean this up
    mod2$runtime = mod$runtime
    if(mod2$na_sdrep==FALSE & !is.na(mod2$na_sdrep)) ok_sdrep2=TRUE else ok_sdrep2=FALSE
    if(ok_sdrep2) saveRDS(mod2, file=here("results","dat_2019","NAA_M_GSI_v5",paste0(df.mods$Model[m],".rds")))   
  }
}

# these converged but don't have peels... get them
# m5 ok
# m14 refitting gives NaN sdrep (borderline convergence), can't get retros or proj
# m30 refitting gives NaN sdrep (borderline convergence), can get retros and proj
for(m in c(5,14,30,35)){
  mod = readRDS(here("results","dat_2019","NAA_M_GSI_v5",paste0("m",m,".rds")))
  new_input <- mod$input
  new_input$par <- mod$env$parList(mod$opt$par) # start at optimized values
  mod2 <- fit_wham(new_input, do.retro=T, do.osa=F, do.proj=T)
  if(exists("err")) rm("err") # need to clean this up
  mod2$runtime = mod$runtime  
  if(!mod2$na_sdrep) saveRDS(mod2, file=here("results","dat_2019","NAA_M_GSI_v5",paste0(df.mods$Model[m],".rds")))   
}

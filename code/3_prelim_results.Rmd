...
# load models
mod.list <- here("results","dat_2019","bias_correct_oe_v2","NAA",paste0(df.mods$Model,".rds"))
...
# load models
mod.list <- here("results","dat_2019","bias_correct_oe_v2","M",paste0(df.mods$Model,".rds"))
mods <- lapply(mod.list, readRDS)
...
mod.list <- here("results","dat_2019","bias_correct_oe_v2","NAA_M",paste0(df.mods$Model,".rds"))
...
# load models
mod.list <- here("results","dat_2019","bias_correct_oe_v2","NAA_M_CPI",paste0(df.mods$Model,".rds"))
mods <- lapply(mod.list, readRDS)
...
# df <- df.mods %>% filter(NAA_cor == "iid", CPI_how==2, CPI_mod=='ar1', pdHess==TRUE)
...
df %>%
  ...
  select(-one_of("pdHess","conv","runtime")) %>% 
  # select(-one_of("pdHess","conv","CPI_how","CPI_mod","runtime")) %>% 
  dplyr::rename("Random effects on NAA"="NAA_cor","Random effects on M"="M_re",
...
# load models
mod.list <- here("results","dat_2019","bias_correct_oe_v2","best",paste0(df.mods$Model,"_proj.rds"))
mods <- lapply(mod.list, readRDS)
...
mod <- readRDS(here("results","dat_2019","bias_correct_oe_v2","best","m8_proj.rds"))
...
mod.list <- c(here("results","dat_2019","bias_correct_oe_v2","NAA","Base.rds"),
              ...
              here("results","dat_2019","bias_correct_oe_v2","NAA_M_CPI","NAA-M-CPI-2.rds"))
mods <- lapply(mod.list, readRDS)
mod.labs <- c("Base","NAA-5", "M-4", "NAA-M-2", "NAA-M-CPI-2")
years = mods[[1]]$years
...
<!-- #               here("results","dat_2019","bias_correct_oe","NAA_M_iid","m5.rds"), -->
<!-- #               here("results","dat_2019","bias_correct_oe","NAA_M_CPI","m22.rds")) -->
<!-- mod.list <- c(here("results","dat_2019","bias_correct_oe","best_zeroFproj","m1_proj.rds"), -->
<!--               here("results","dat_2019","bias_correct_oe","best_zeroFproj","m2_proj.rds"), -->
<!--               here("results","dat_2019","bias_correct_oe","best_zeroFproj","m3_proj.rds"), -->
<!--               here("results","dat_2019","bias_correct_oe","best_zeroFproj","m5_proj.rds"), -->
<!--               here("results","dat_2019","bias_correct_oe","best_zeroFproj","m6_proj.rds"), -->
<!--               here("results","dat_2019","bias_correct_oe","best_zeroFproj","m7_proj.rds"), -->
<!--               here("results","dat_2019","bias_correct_oe","best_zeroFproj","m8_proj.rds")) -->
<!-- mods <- lapply(mod.list, readRDS) -->
<!-- mod.labs <- c("NAA-1","NAA-3","NAA-6","M-5","NAA-M-2","NAA-M-5","NAA-M-CPI-2") -->
...
<!-- df$Model <- factor(df$Model, levels=mod.labs, labels=mod.labs) -->
<!-- df$Year <- as.integer(df$Year) -->
<!-- # df[which(df$var=="SSB~/~SSB[MSY]"),3:5] > 1 -->
<!-- dat <- data.table(df) -->
<!-- dat[,y_min := 0, by = var] -->
<!-- dat <- filter(dat, var=="SSB") -->

<!-- ggplot(dat, aes(x=Year, y=val, color=Model, group=Model)) + -->
<!--   geom_ribbon(aes(ymin=lo, ymax=hi, fill=Model), color=NA, alpha=.15) + -->
<!--   geom_line(size=.8) + -->
<!--   geom_vline(xintercept = tail(mods[[1]]$years,1), linetype=2, size=.4) + -->
<!--   facet_grid(rows=vars(var), cols=vars(Model), scales="free_y", labeller = label_parsed) + -->
<!--   # ylab(NULL) + -->
<!--   geom_blank(aes(y = y_min)) + -->
<!--   scale_y_continuous(expand=c(0.01,0.01)) + -->
<!--   # coord_cartesian(xlim=c(1995,2021)) + -->

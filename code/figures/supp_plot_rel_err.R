# Plot relative error from simulation tests
...
plot_rel_err <- function(results, stock.id="SNEMAYT", re="NAA", bc.type=2, sim.types=1:2, multipanel=TRUE, plot.eps=FALSE,
  ...
  custom_boxplot_stat <- function(x) {
  ...
  if(multipanel){
    ...
    # collapse across years, group by om/em
    	df.plot <- filter(results, type==levels(results$type)[ty]) %>%
    	  ...
    	  select(om, em, SSB.rel, F.rel, relB.rel, relF.rel, R.rel) %>% as.data.frame
    	df.plot <- do.call(data.frame, lapply(df.plot, function(x) replace(x, is.infinite(x), NA)))
    	# df.plot[df.plot == 0] = NA
    	df.plot <- df.plot %>%
    	  pivot_longer(-c(om,em), names_to = "variable", values_to = "val") %>%
    	  group_by(om, em, variable) %>%
    	...
    	df.plot$variable <- factor(df.plot$variable, levels=c("SSB.rel", "F.rel", "relB.rel", "relF.rel", "R.rel"), 
    	 ...
    	 p <- ggplot(df.plot, aes(x=em, y=rel_err_mean)) +
    	  ...
    	  geom_point(aes(fill=em), shape=21, size=3) +
    	   # scale_fill_jco(name="", labels=lapply(levels(df.plot$em), function(x) parse(text=x))) +
    	   scale_fill_jco() +
    	   coord_cartesian(ylim=c(-.5,.5)) +
    	  facet_grid(rows=vars(variable), cols=vars(om), labeller = label_parsed) +
    	  theme_bw() +
    	  # theme(legend.position="bottom",
    	  theme(legend.position="none",
    	        strip.text.y = element_text(size = 10), strip.text.x = element_text(size = 10, margin = margin(3,1,1,1, "pt")),
    	        axis.title.x = element_text(size = 12), axis.text.x = element_text(size = 8, angle = 45, hjust = 1),
    ...
    for(ty in sim.types){
      ...
      p <- ggplot(df.plot, aes(x=year, y=SSB.rel-1)) +
        ...
        geom_hline(yintercept = 0, linetype=2, color='black') +
        facet_grid(rows=vars(em), cols=vars(om), labeller = label_parsed) +
        theme_bw() +
        theme(axis.text.x = element_text(size=8), plot.margin = unit(c(0.3,0.3,0.1,0.1), "in"))
      png(file.path(plots_dir,paste0("1_",re,"_ssb_",types[ty],".png")), width=7, height=7, units='in',res=100)
      print(p)
      ...
      #   theme_bw() +
      #   theme(axis.text.x = element_text(size=8), plot.margin = unit(c(0.3,0.3,0.1,0.1), "in"))
      # png(file.path(plots_dir,paste0("1_",re,"_ssb_",types[ty],"_medianCI.png")), width=7, height=7, units='in',res=100)
      # print(p)
      # grid::grid.text(unit(0.98,"npc"),0.5, label = 'Estimation model', rot = 270) # right
      # grid::grid.text(unit(0.5,"npc"),unit(.98,'npc'), label = 'Operating model', rot = 0)   # top)
      # dev.off()      

      # png(file.path(plots_dir,paste0("1_",re,"_ssb_boxplots",types[ty],".png")), width=8, height=3, units='in',res=100)
      ...
      p <- df.plot %>% 
        ...
        mutate(rel_err_lo = rel_err_mean - 1.96*rel_err_se, rel_err_hi = rel_err_mean + 1.96*rel_err_se) %>%         
        # mutate(rel_err_lo2 = rel_err_mean - 0.674*rel_err_se, rel_err_hi2 = rel_err_mean + 0.674*rel_err_se) %>%
        ggplot(aes(x=em, y=rel_err_mean)) +
        geom_hline(yintercept = 0, linetype=2, color='black') +
        geom_linerange(aes(ymin=rel_err_lo, ymax=rel_err_hi), size=.5) +
        ...
        theme_bw() +
        theme(plot.title = element_text(hjust = 0.5))
      png(file.path(plots_dir,paste0("1_",re,"_ssb_boxplots",types[ty],"_medianCI.png")), width=8, height=3, units='in',res=100)
      print(p)
      dev.off()
      ...
      p <- ggplot(df.plot, aes(x=year, y=F.rel-1)) +
        ...
        theme_bw() +
        theme(axis.text.x = element_text(size=8), plot.margin = unit(c(0.3,0.3,0.1,0.1), "in"))
      png(file.path(plots_dir,paste0("2_",re,"_F_",types[ty],".png")), width=7, height=7, units='in',res=100)
      print(p)
      grid::grid.text(unit(0.98,"npc"),0.5, label = 'Estimation model', rot = 270) # right
      grid::grid.text(unit(0.5,"npc"),unit(.98,'npc'), label = 'Operating model', rot = 0)   # top)
      dev.off()
      ...
      png(file.path(plots_dir,paste0("2_",re,"_F_boxplots",types[ty],".png")), width=8, height=3, units='in',res=100)
      print(ggplot(df.plot, aes(x=em, y=F.rel-1)) +
              # geom_boxplot(aes(fill=em), outlier.shape = NA) +
              stat_summary(aes(fill=em), fun.data=custom_boxplot_stat, geom="boxplot") + 
        scale_fill_jco(name="Estimation model") +
        coord_cartesian(ylim=c(-.5,.5)) +
      ...
      p <- df.plot %>% 
        ...
        theme_bw() +
        theme(plot.title = element_text(hjust = 0.5))
      png(file.path(plots_dir,paste0("2_",re,"_F_boxplots",types[ty],"_medianCI.png")), width=8, height=3, units='in',res=100)
      print(p)
      dev.off()
      ...
      p <- df.plot %>%
        ...
        theme_bw() +
        theme(axis.text.x = element_text(size=8), plot.margin = unit(c(0.3,0.3,0.1,0.1), "in"))
      png(file.path(plots_dir,paste0("3_",re,"_relB_",types[ty],".png")), width=7, height=7, units='in',res=100)
      print(p)
      grid::grid.text(unit(0.98,"npc"),0.5, label = 'Estimation model', rot = 270) # right
      grid::grid.text(unit(0.5,"npc"),unit(.98,'npc'), label = 'Operating model', rot = 0)   # top)
      dev.off()
      
      png(file.path(plots_dir,paste0("3_",re,"_relB_boxplots",types[ty],".png")), width=8, height=3, units='in',res=100)
      print(ggplot(df.plot, aes(x=em, y=relB.rel-1)) +
              # geom_boxplot(aes(fill=em), outlier.shape = NA) +
              stat_summary(aes(fill=em), fun.data=custom_boxplot_stat, geom="boxplot") + 
        scale_fill_jco(name="Estimation model") +
        coord_cartesian(ylim=c(-.5,.5)) +
      ...
      p <- df.plot %>% 
        ...
        theme_bw() +
        theme(plot.title = element_text(hjust = 0.5))
      png(file.path(plots_dir,paste0("3_",re,"_relB_boxplots",types[ty],"_medianCI.png")), width=8, height=3, units='in',res=100)
      print(p)
      dev.off()
      ...
      p <- df.plot %>%
        ...
        theme_bw() +
        theme(axis.text.x = element_text(size=8), plot.margin = unit(c(0.3,0.3,0.1,0.1), "in"))
      png(file.path(plots_dir,paste0("4_",re,"_relF_",types[ty],".png")), width=7, height=7, units='in',res=100)
      print(p)
      grid::grid.text(unit(0.98,"npc"),0.5, label = 'Estimation model', rot = 270) # right
      grid::grid.text(unit(0.5,"npc"),unit(.98,'npc'), label = 'Operating model', rot = 0)   # top)
      dev.off()
      
      png(file.path(plots_dir,paste0("4_",re,"_relF_boxplots",types[ty],".png")), width=8, height=3, units='in',res=100)
      print(ggplot(df.plot, aes(x=em, y=relF.rel-1)) +
              # geom_boxplot(aes(fill=em), outlier.shape = NA) +
              stat_summary(aes(fill=em), fun.data=custom_boxplot_stat, geom="boxplot") + 
        scale_fill_jco(name="Estimation model") +
        coord_cartesian(ylim=c(-.5,.5)) +
      ...
      p <- df.plot %>% 
        ...
        theme_bw() +
        theme(plot.title = element_text(hjust = 0.5))
      png(file.path(plots_dir,paste0("4_",re,"_relF_boxplots",types[ty],"_medianCI.png")), width=8, height=3, units='in',res=100)
      print(p)
      dev.off()
      ...
      p <- ggplot(df.plot, aes(x=year, y=catch.rel-1)) +
        ...
        theme_bw() +
        theme(axis.text.x = element_text(size=8), plot.margin = unit(c(0.3,0.3,0.1,0.1), "in"))
      png(file.path(plots_dir,paste0("5_",re,"_catch_",types[ty],".png")), width=7, height=7, units='in',res=100)
      print(p)
      grid::grid.text(unit(0.98,"npc"),0.5, label = 'Estimation model', rot = 270) # right
      grid::grid.text(unit(0.5,"npc"),unit(.98,'npc'), label = 'Operating model', rot = 0)   # top)
      dev.off()
      ...
      png(file.path(plots_dir,paste0("5_",re,"_catch_boxplots",types[ty],".png")), width=8, height=3, units='in',res=100)
      print(ggplot(df.plot, aes(x=em, y=catch.rel-1)) +
              # geom_boxplot(aes(fill=em), outlier.shape = NA) +
              stat_summary(aes(fill=em), fun.data=custom_boxplot_stat, geom="boxplot") + 
        scale_fill_jco(name="Estimation model") +
        coord_cartesian(ylim=c(-.5,.5)) +
      ...
      p <- df.plot %>% 
        ...
        theme_bw() +
        theme(plot.title = element_text(hjust = 0.5))
      png(file.path(plots_dir,paste0("5_",re,"_catch_boxplots",types[ty],"_medianCI.png")), width=8, height=3, units='in',res=100)
      print(p)
      dev.off()
      ...
      p <- ggplot(df.plot, aes(x=year, y=R.rel-1)) +
        ...
        theme_bw() +
        theme(axis.text.x = element_text(size=8), plot.margin = unit(c(0.3,0.3,0.1,0.1), "in"))
      png(file.path(plots_dir,paste0("6_",re,"_R_",types[ty],".png")), width=7, height=7, units='in',res=100)
      print(p)
      grid::grid.text(unit(0.98,"npc"),0.5, label = 'Estimation model', rot = 270) # right
      grid::grid.text(unit(0.5,"npc"),unit(.98,'npc'), label = 'Operating model', rot = 0)   # top)
      dev.off()

      png(file.path(plots_dir,paste0("6_",re,"_R_boxplots",types[ty],".png")), width=8, height=3, units='in',res=100)
      print(ggplot(df.plot, aes(x=em, y=R.rel-1)) +
              # geom_boxplot(aes(fill=em), outlier.shape = NA) +
              stat_summary(aes(fill=em), fun.data=custom_boxplot_stat, geom="boxplot") + 
        scale_fill_jco(name="Estimation model") +
        coord_cartesian(ylim=c(-.5,.5)) +
      ...
      p <- df.plot %>% 
        ...
        theme_bw() +
        theme(plot.title = element_text(hjust = 0.5))
      png(file.path(plots_dir,paste0("6_",re,"_R_boxplots",types[ty],"_medianCI.png")), width=8, height=3, units='in',res=100)
      print(p)
      dev.off()
...
# ---------------------------------------------------------------------------
      ...
      if(plot.eps){
        ...
        png(file.path(plots_dir,paste0("1_",re,"_ssb_",types[ty],"_bc.png")), width=7, height=7, units='in',res=100)
        print(p)
        grid::grid.text(unit(0.98,"npc"),0.5, label = 'Estimation model', rot = 270) # right
        grid::grid.text(unit(0.5,"npc"),unit(.98,'npc'), label = 'Operating model', rot = 0)   # top)
        dev.off()

        png(file.path(plots_dir,paste0("1_",re,"_ssb_boxplots",types[ty],"_bc.png")), width=8, height=3, units='in',res=100)
        print(ggplot(df.plot, aes(x=em, y=SSB.rel.bc-1)) +
          geom_boxplot(aes(fill=em), outlier.shape = NA) +
          scale_fill_jco(name="Estimation model") +
          coord_cartesian(ylim=c(-.5,.5)) +
          xlab("Estimation model") +
        ...
        png(file.path(plots_dir,paste0("2_",re,"_F_",types[ty],"_bc.png")), width=7, height=7, units='in',res=100)
        print(p)
        grid::grid.text(unit(0.98,"npc"),0.5, label = 'Estimation model', rot = 270) # right
        grid::grid.text(unit(0.5,"npc"),unit(.98,'npc'), label = 'Operating model', rot = 0)   # top)
        dev.off()

        png(file.path(plots_dir,paste0("2_",re,"_F_boxplots",types[ty],"_bc.png")), width=8, height=3, units='in',res=100)
        print(ggplot(df.plot, aes(x=em, y=F.rel.bc-1)) +
          geom_boxplot(aes(fill=em), outlier.shape = NA) +
          scale_fill_jco(name="Estimation model") +
          coord_cartesian(ylim=c(-.5,.5)) +
          xlab("Estimation model") +
        ...
        png(file.path(plots_dir,paste0("3_",re,"_relB_",types[ty],"_bc.png")), width=7, height=7, units='in',res=100)
        print(p)
        grid::grid.text(unit(0.98,"npc"),0.5, label = 'Estimation model', rot = 270) # right
        grid::grid.text(unit(0.5,"npc"),unit(.98,'npc'), label = 'Operating model', rot = 0)   # top)
        dev.off()

        png(file.path(plots_dir,paste0("3_",re,"_relB_boxplots",types[ty],"_bc.png")), width=8, height=3, units='in',res=100)
        print(ggplot(df.plot, aes(x=em, y=relB.rel.bc-1)) +
          geom_boxplot(aes(fill=em), outlier.shape = NA) +
          scale_fill_jco(name="Estimation model") +
          coord_cartesian(ylim=c(-.5,.5)) +
          xlab("Estimation model") +
        ...
        p <- ggplot(df.plot, aes(x=year, y=relF.rel.bc-1)) +
          ...
          theme_bw() +
          theme(axis.text.x = element_text(size=8), plot.margin = unit(c(0.3,0.3,0.1,0.1), "in"))
        png(file.path(plots_dir,paste0("4_",re,"_relF_",types[ty],"_bc.png")), width=7, height=7, units='in',res=100)
        print(p)
        grid::grid.text(unit(0.98,"npc"),0.5, label = 'Estimation model', rot = 270) # right
        grid::grid.text(unit(0.5,"npc"),unit(.98,'npc'), label = 'Operating model', rot = 0)   # top)
        dev.off()

        png(file.path(plots_dir,paste0("4_",re,"_relF_boxplots",types[ty],"_bc.png")), width=8, height=3, units='in',res=100)
        print(ggplot(df.plot, aes(x=em, y=relF.rel.bc-1)) +
          geom_boxplot(aes(fill=em), outlier.shape = NA) +
          scale_fill_jco(name="Estimation model") +
          coord_cartesian(ylim=c(-.5,.5)) +
          xlab("Estimation model") +
        ...
        p <- ggplot(df.plot, aes(x=year, y=catch.rel.bc-1)) +
          ...
          theme_bw() +
          theme(axis.text.x = element_text(size=8), plot.margin = unit(c(0.3,0.3,0.1,0.1), "in"))
        png(file.path(plots_dir,paste0("5_",re,"_catch_",types[ty],"_bc.png")), width=7, height=7, units='in',res=100)
        print(p)
        grid::grid.text(unit(0.98,"npc"),0.5, label = 'Estimation model', rot = 270) # right
        grid::grid.text(unit(0.5,"npc"),unit(.98,'npc'), label = 'Operating model', rot = 0)   # top)
        dev.off()

        png(file.path(plots_dir,paste0("5_",re,"_catch_boxplots",types[ty],"_bc.png")), width=8, height=3, units='in',res=100)
        print(ggplot(df.plot, aes(x=em, y=catch.rel.bc-1)) +
          geom_boxplot(aes(fill=em), outlier.shape = NA) +
          scale_fill_jco(name="Estimation model") +
          coord_cartesian(ylim=c(-.5,.5)) +
          xlab("Estimation model") +
        ...
        p <- ggplot(df.plot, aes(x=year, y=R.rel.bc-1)) +
          ...
          theme_bw() +
          theme(axis.text.x = element_text(size=8), plot.margin = unit(c(0.3,0.3,0.1,0.1), "in"))
        png(file.path(plots_dir,paste0("6_",re,"_R_",types[ty],"_bc.png")), width=7, height=7, units='in',res=100)
        print(p)
        grid::grid.text(unit(0.98,"npc"),0.5, label = 'Estimation model', rot = 270) # right
        grid::grid.text(unit(0.5,"npc"),unit(.98,'npc'), label = 'Operating model', rot = 0)   # top)
        dev.off()

        png(file.path(plots_dir,paste0("6_",re,"_R_boxplots",types[ty],"_bc.png")), width=8, height=3, units='in',res=100)
        print(ggplot(df.plot, aes(x=em, y=R.rel.bc-1)) +
          geom_boxplot(aes(fill=em), outlier.shape = NA) +
          scale_fill_jco(name="Estimation model") +
          coord_cartesian(ylim=c(-.5,.5)) +
          xlab("Estimation model") +

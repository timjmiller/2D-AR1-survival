...
# source("/home/bstock/Documents/ms/2D-AR1-survival/code/figures/fig2_reldiff_2dar1.R")
...
mod.list <- c(here("results","dat_2019","bias_correct_oepe_rev","NAA","NAA-2.rds"),
              ...
              here("results","dat_2019","bias_correct_oepe_rev","NAA_CPI","NAA-CPI-1.rds"),
              here("results","dat_2019","bias_correct_oepe_rev","NAA_CPI","NAA-CPI-4.rds"))
mods <- lapply(mod.list, readRDS)
mod.cols <- c("IID","2D AR(1)", "NAA + M", "IID","2D AR(1)", "NAA + M","IID + CPI","2D AR(1) + CPI")
mod.labs <- c("NAA","NAA","NAA","M","M","M","NAA","NAA")
...
df$Col <- factor(df$Col, levels=c("IID","IID + CPI","2D AR(1)", "2D AR(1) + CPI", "NAA + M"), labels=c("Indep.","Indep. + CPI","2D AR(1)", "2D AR(1) + CPI", "2D AR(1) + NAA/M"))
df$lty <- 1
